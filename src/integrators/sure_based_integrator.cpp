...
SUREBasedIntegrator *CreateSUREBasedIntegrator(
    ...
    return new SUREBasedIntegrator(sampler, camera, std::shared_ptr<PathIntegrator>(path_integrator), pixel_bounds);
}
...
void SUREBasedIntegrator::Render(const Scene& scene) {
    ...
    ProgressReporter reporter(num_tiles.x * num_tiles.y, "SURE-Based Integrator rendering");
    {
        ParallelFor2D([&](Point2i tile) {
            ...
            for (Point2i pixel: tile_bounds) {
                ...
                do {
                    CameraSample camera_sample = tile_sampler->GetCameraSample(pixel);

                    RayDifferential ray;
                    Float ray_weight =
                        camera->GenerateRayDifferential(camera_sample, &ray);
                    ray.ScaleDifferentials(
                        1 / std::sqrt((Float)tile_sampler->samplesPerPixel));
                    Spectrum L(0.f);
                    if (ray_weight > 0) {
                        L = path_integrator->Li(ray, scene, *tile_sampler, arena, 0);
                    }
                    ...
                    film_tile->AddSample(camera_sample.pFilm, L, ray_weight);
